...
def generate_emergency_data(device_id, timestamp, loc):
    return {
        'id' : uuid.uuid4(),
        'device_id' : device_id,
...
def generate_weather_data(device_id, timestamp, loc):
    return {
        'id' : uuid.uuid4(),
        'device_id' : device_id,
        'location' : loc ,
...
def generate_vehicle_data(device_id):
    location = simualte_vehicle_movement()
...
def simulate_journey(producer, device_id):
    while True:
        ...
        if (vehicle_data['location'][0] >= BIRMINGHAM_COORDINATES['latitude'] 
            ...
            print('vehicle has reached . Simulation ended')
            break
...
if __name__ == "__main__":
    ...
    try:
        simulate_journey(producer, 'Vehicle-Sam')
    except KeyboardInterrupt:
        print('Simualtion ended by user')
